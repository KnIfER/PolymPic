...
import static com.knaiver.polymer.WebCompoundListener.ensureMarkJS;
import static com.knaiver.polymer.widgets.Utils.getWindowManagerViews;
...
public class WebViewmy extends WebView implements MenuItem.OnMenuItemClickListener {
	...
	private final static int[] DuiYinChengSanRen=new int[]{R.id.toolbar_action0, R.id.toolbar_action1, R.id.toolbar_action3};
	private static String[] MenuItemsToReMove;
...
//			}
	...
	private class AdvancedWebViewCallback extends ActionMode.Callback2 {
		...
		public void TweakWebviewContextMenu(Menu menu) {
			...
			MenuItemsToReMove = new String[]{getShareText(), getSelectText()};
			...
			for(int i=0;i<menu.size();i++) {
				...
				for (int j = 0; j < MenuItemsToReMove.length; j++) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if(webviewcallback!=null) {
			if( action==MotionEvent.ACTION_MOVE ) {
				if(bIsActionMenuShownNew&&(Math.abs(lastY-orgY)>GlobalOptions.density*5||Math.abs(lastX-orgX)>GlobalOptions.density*5)) {
					bIsActionMenuShownNew=false;
